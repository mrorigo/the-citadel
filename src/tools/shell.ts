...
export const runCommandTool = {
    ...
    schema: z.object({
        command: z.union([z.string(), z.array(z.string())]).optional().describe('The shell command to execute as a single string (e.g., "ls -la", "npm test")'),
        cmd: z.union([z.string(), z.array(z.string())]).optional().describe('Alternative: command as string or array of arguments'),
    }).passthrough(),
    handler: async (args: { command?: string | string[]; cmd?: string | string[];[key: string]: unknown }) => {
        // Normalize: accept both 'command' and 'cmd', convert arrays to strings
        let command: string | undefined;
        if (args.command) {
            command = Array.isArray(args.command) ? args.command.join(' ') : args.command;
        } else if (args.cmd) {
