...
export async function loadConfig(): Promise<CitadelConfig> {
	const existing = getGlobalSingleton<CitadelConfig | null>(
		CONFIG_KEY,
		() => null,
	);
	...
	const rawConfig = {
		...
		providers: {
			...
			ollama: {
				baseURL:
					process.env.CITADEL_OLLAMA_BASE_URL ||
					userConfig.providers?.ollama?.baseURL,
				apiKey:
					process.env.CITADEL_OLLAMA_API_KEY ||
					userConfig.providers?.ollama?.apiKey,
				...userConfig.providers?.ollama,
			},
			openai: {
				apiKey:
					process.env.OPENAI_API_KEY || userConfig.providers?.openai?.apiKey,
				...userConfig.providers?.openai,
			},
			anthropic: {
				apiKey:
					process.env.ANTHROPIC_API_KEY ||
					userConfig.providers?.anthropic?.apiKey,
...
export function getConfig(): CitadelConfig {
	const config = getGlobalSingleton<CitadelConfig | null>(
		CONFIG_KEY,
		() => null,
	);
	if (!config) {
