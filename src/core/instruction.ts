...
export class RoleProvider implements InstructionProvider {
    ...
    async getInstructions(ctx: InstructionContext): Promise<string | null> {
        const path = resolve(process.cwd(), `.citadel/instructions/role-${ctx.role}.md`);
        if (existsSync(path)) {
            try {
...
export class InstructionService {
    ...
    async buildPrompt(ctx: InstructionContext, basePrompt: string): Promise<string> {
        ...
        for (const provider of this.providers) {
            try {
                ...
                logger.error(`[InstructionService] Provider ${provider.name} failed:`, err);
            }
        }
