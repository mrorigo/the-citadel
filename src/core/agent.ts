    ...
    protected async checkPermissions(toolName: string, args: any): Promise<{ allowed: boolean; error?: string }> {
        ...
        const matches = (path: string, patterns: string[]) => {
            ...
            for (const pattern of patterns) {
                ...
                // Check relative path from context root
                // Assuming context source path is where the rules apply relative to.
                // path is likely absolute.
                // We need strict matching. let's just attempt match on the path string relative to cwd.
                if (glob.match(path)) return true; // Naive match
