...
const BeadNode = ({ node, depth }: { node: TreeNode; depth: number }) => {
    ...
    const color =
        node.bead.status === 'done' ? 'green' :
            node.bead.status === 'verify' ? 'magenta' :
                node.bead.status === 'in_progress' ? 'yellow' : 'white';
    ...
    return (
        <Box flexDirection="column">
            <Text color={color}>
                {indent}{icon} {node.bead.title} <Text color="gray">({node.bead.id})</Text>
            </Text>
...
export const MoleculeTree = () => {
    ...
    return (
        <Box flexDirection="column" borderStyle="round" borderColor="magenta" width="50%" height={15} overflowY="hidden">
