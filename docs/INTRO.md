To the Agent tasked with implementing **The Foundry**:
...
**Foundry Equivalent:** The Ledger / Durable State Store
...
For The Foundry, Beads provides three critical architectural guarantees:
...
**Implementation Directive:** You will utilize the Beads data structure but enforce strict schema validation. Where Gas Town allows agents to write loose JSON, The Foundry requires rigid adherence to a defined state machine (e.g., `Pending` $\to$ `In_Progress` $\to$ `Verify` $\to$ `Done`).

***

### Part 2: The Logic (Gas Town Patterns)
**Concept:** Nondeterministic Idempotence & The MEOW Stack
**Foundry Equivalent:** Deterministic DAG Execution
...
**Implementation Directive:** Gas Town "vibecodes" these molecules, often allowing agents to hallucinate steps. The Foundry must formalize "Molecules" into **Deterministic DAGs** defined in TOML/Formulas. An agent cannot proceed to node $B$ until node $A$ passes a verifiable acceptance test.

***

### Part 3: The Roles (Service Architecture)
**Concept:** Specialized Agent Roles
**Foundry Equivalent:** Micro-Service Agents

Gas Town anthropomorphizes distributed system roles into "characters." To build The Foundry, you must strip the role-playing and implement them as strict services.

| Gas Town Role    | Function                                                              | Foundry Implementation                                                                                                     |
| :--------------- | :-------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------- |
...
1.  **Avoid "Seances":** Gas Town agents talk to "dead ancestors" (previous sessions) to regain context.
    *   *Foundry Fix:* No communication between sessions. Context must be fully serializable to the Bead.
2.  **Avoid "Vibecoding" Merges:** The Gas Town "Refinery" will rewrite code arbitrarily to make it merge.
    *   *Foundry Fix:* Stacked Diffs. If a merge conflict exists, the ticket is rejected, and a worker is respawned to rebase and fix it.
3.  **Avoid Infinite Loops:** Gas Town relies on "nudging" agents to wake them up.
    *   *Foundry Fix:* Active Queue Consumption. The system instantiates workers; workers do not "decide" to work.

### Summary
Gas Town is the **prototype**: a chaotic, "Mad Max" demonstration that Git-backed agent swarms can work.
The Foundry is the **product**: a factory where "Polecats" become interchangeable compute units processing a durable state machine.
