...
- **Intervention:** Allow the user to pause, cancel, or modify Pearls directly from the UI.
...
- **Failure Handlers:** `on_failure` routes to specialized recovery pearls.
...
- **Gateway Pearls:** A specific pearl assigned to the *User* (not an Agent).
- **Blocking Mechanism:** Critical tasks depend on this Gateway Pearl (`bd dep add`).
...
- **Tech:** Simple local vector store (e.g., `sqlite-vss` or just embeddings stored in pearls).
