...
This architecture strictly separates **The Citadel** (The Application Logic) from **Pearls** (The Database). We implement the "MEOW Stack" concepts (Formulas, Molecules, Convoys) purely through orchestration logic and standard `prl` commands.
...
The Router will act as a "Compiler," translating static TOML templates (Formulas) into standard `prl create` and `prl dep add` commands. This allows us to support complex, generic workflows (Molecules) and high-level tracking (Convoys) using existing infrastructure.

---

## 2. Feature: Generic Formula Support (TOML)
We will implement a standard library of workflows stored as TOML files. These files are not read by `prl`, but by the **Citadel Router**.
...
We cannot add a `prl cook` command. Therefore, we will implement the cooking logic inside the **Router Agent's runtime**.
...
2.  **Create Container (The Molecule):**
    *   *Command:* `prl create "Run system_migration (Auth)" --type epic`
    *   *Capture:* Store the resulting ID (e.g., `prl-100`).
3.  **Instantiate Steps:**
    *   Iterate through `[[steps]]`.
    *   *Command:* `prl create "{title}" --description "{description}" --parent prl-100`
    *   *Map:* Store the mapping of `TOML_ID` $\to$ `REAL_PEARL_ID`.
4.  **Wire Dependencies (The DAG):**
    *   For every step with a `needs` array:
    *   *Command:* `prl dep add {REAL_CHILD_ID} {REAL_PARENT_ID}`
...
We cannot add `prl convoy create`. We will instead define a **Convoy** as a specific *Type* of Pearl managed by conventions.
...
*   **Creation:** The Router creates Convoys using the generic `type` flag (supported by standard JSONL schema in Pearls).
    *   *Command:* `prl create "Q1 Cleanup Convoy" --type convoy`
*   **Tracking:** To view Convoys, we will not use a new command. We will filter standard output.
    *   *Command:* `prl show --type convoy` (or standard `grep` on the JSONL if `prl show` filters are limited).
*   **Assignment:** When "slinging" a Molecule, the Router will parent the Molecule's root Epic to the Convoy Pearl.
    *   *Result:* `Convoy (prl-1)` $\to$ `Molecule Epic (prl-2)` $\to$ `Task (prl-3)`.
...
We do not need new `prl` commands. We simply grant **Worker Agents** the permission to recursively call `prl create` and `prl dep add`.
...
3.  **Worker** executes a script (or manual CLI commands):
    *   `prl create "Subtask 1" --parent {Task_A_ID}`
    *   `prl create "Subtask 2" --parent {Task_A_ID}`
    *   `prl dep add {Task_A_ID} {Subtask_1_ID}` (Wait logic: Task A cannot close until Subtask 1 closes).
4.  **Evaluator** Update: The Evaluator (QA agent) must check `prl ready` logic. If `Task A` has open children/dependencies, it refuses to merge/close it.
...
| **Instantiation**     | `prl cook` / `prl pour`  | Router loop executing `prl create` + `prl dep add` |
| **Workflow Object**   | "Molecule" Object        | Standard **Epic** Pearl (`type=epic`)              |
| **Shipment Tracking** | `prl convoy create`      | Standard Pearl with `type=convoy`                  |
| **Runtime Expansion** | "Protomolecule" Macros   | Worker recursively running `prl create`            |
