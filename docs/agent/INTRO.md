...
The following introduction deconstructs the Gas Town stack into its primitive components: **Pearls** (The Data Plane) and **Gas Town Orchestration** (The Control Plane).

***

### Part 1: The Substrate (Pearls)
**Concept:** Distributed, Git-Backed State
**Citadel Equivalent:** The Ledger / Durable State Store

At the heart of the system is **Pearls**, a "memory upgrade" for agents. It acts as a distributed graph issue tracker where every unit of work is stored as a JSON object in a flat text file (`.pearls/`), versioned alongside the code in Git.

For The Citadel, Pearls provides three critical architectural guarantees:
1.  **Decoupling State from Compute:** Agents are ephemeral. When a session dies, the context is not lost; it is preserved in the Pearl. This allows agents to be treated as "cattle" (disposable) rather than "pets" (long-lived sessions).
...
3.  **Conflict-Free Addressing:** Pearls uses hash-based IDs (e.g., `bd-a1b2`) to prevent merge collisions when multiple agents work on parallel branches.

**Implementation Directive:** You will utilize the Pearls data structure but enforce strict schema validation. Where Gas Town allows agents to write loose JSON, The Citadel requires rigid adherence to a defined state machine (e.g., `Pending` $\to$ `In_Progress` $\to$ `Verify` $\to$ `Done`).
...
*   **The Principle:** The *path* an agent takes to solve a problem is probabilistic (nondeterministic), but the *outcome*—checking off a Pearl—is durable and binary.
...
1.  **Pearls:** Atomic tasks.
2.  **Molecules:** Chains of pearls that form workflows (e.g., Plan $\to$ Code $\to$ Test).
...
| :--------------- | :-------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------- |
| **The Mayor**    | The Router/Concierge. Receives high-level intent and breaks it down.  | **The Router:** A read-only service that compiles "Specs" into ticket trees (Pearls).                                       |
...
1.  **Avoid "Seances":** Gas Town agents talk to "dead ancestors" (previous sessions) to regain context.
    *   *Citadel Fix:* No communication between sessions. Context must be fully serializable to the Pearl.
...
Proceed with the architecture defined in the PRD, utilizing Pearls as your immutable ledger.